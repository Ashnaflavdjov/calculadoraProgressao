...
plus.onclick = () => {
    ...
    if(lastSeqNum){
        ...
        numInput.value = result.sequence[0] * (result.razao ** (sequenceForm.elements.length));
...
function GetNthTerm(result, numero){
    ...
    return result.sequence[0] * (result.razao ** (numero - 1));
}
...
function SumTerms(result, numero){
    ...
    return (result.sequence[0] * (result.razao ** numero - 1)) / (result.razao - 1);
}
